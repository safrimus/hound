...
def get_paged_url_from_git_api(number_of_page):
    for page in range(1, int(number_of_page) + 1):
        ...
        for repo in response:
            ...
            repo_vcs_config = {'ref': repo_default_branch}
            repo_info = {'url': repo_ssh_url, 'vcs-config': repo_vcs_config, 'ms-between-poll': 3600000,
                         'exclude-dot-files': True}
            repo_list[repo_name] = repo_info
...
if __name__ == "__main__":
    ...
    config_file.write(json.dumps(hound_config, separators=(',', ': '), indent=2, sort_keys=True))
    config_file.close()
